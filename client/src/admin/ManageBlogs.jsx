...
const ManageBlogs = () => {
  const { blogs, setBlogs } = useData(); // ✅ Global Blogs
  ...
  const [postForm, setPostForm] = useState({ title: "", category: "Development", time: "", image: "", url: "" });
  ...
  const handleDelete = (id) => {
    if (window.confirm("Are you sure?")) {
      if (activeTab === "posts") setBlogs(blogs.filter(p => p.id !== id)); 
      else setFaqs(faqs.filter(f => f.id !== id));
    }
  };
  ...
  const handleSubmit = (e) => {
    ...
    if (activeTab === "posts") {
      if (editingId) {
        setBlogs(blogs.map(p => (p.id === editingId ? { ...postForm, id: editingId } : p)));
      } else {
        setBlogs([...blogs, { ...postForm, id: Date.now() }]); 
      }
    } else {
      if (editingId) setFaqs(faqs.map(f => (f.id === editingId ? { ...faqForm, id: editingId } : f)));
    ...
    <div className="relative pb-20">
        ...
        <div className="flex bg-white p-1 rounded-xl border border-gray-200 shadow-sm">
          <button 
            ...
            className={`px-6 py-2 rounded-lg font-bold text-sm transition-all ${
              activeTab === "posts" 
                ? "bg-blue-600 text-white shadow-md" // ✅ Blue active state
      ...
      {activeTab === "posts" && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {blogs.map((post) => (
            <div key={post.id} className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden flex flex-col h-full hover:shadow-md transition-shadow">
              <div className="relative h-48 bg-gray-100">
                <img src={post.image} alt={post.title} className="w-full h-full object-cover" />
                <div className="absolute top-2 right-2 bg-white/90 px-2 py-1 rounded text-xs font-bold text-slate-800">{post.category}</div>
              </div>
              <div className="p-5 flex flex-col flex-grow">
                <div className="flex items-center gap-2 text-xs text-gray-500 mb-2"><Clock size={14} /> {post.time}</div>
                <h3 className="text-lg font-bold text-slate-900 mb-4 line-clamp-2">{post.title}</h3>
                
                <div className="mt-auto grid grid-cols-2 gap-3 pt-4 border-t border-gray-50">
                  <button onClick={() => openEditForm(post)} className="py-2 px-4 rounded-lg bg-blue-50 text-blue-600 font-bold hover:bg-blue-100 flex items-center justify-center gap-2"><Edit size={16} /> Edit</button>
                  <button onClick={() => handleDelete(post.id)} className="py-2 px-4 rounded-lg bg-red-50 text-red-600 font-bold hover:bg-red-100 flex items-center justify-center gap-2"><Trash2 size={16} /> Delete</button>
                </div>
              </div>
            </div>
          ))}
      ...
      {activeTab === "faqs" && (
          ...
          {faqs.map((faq) => (
            <div key={faq.id} className="bg-white p-6 rounded-2xl shadow-sm border border-gray-200 flex justify-between items-start gap-4">
              <div className="flex-1">
                <div className="flex items-center gap-2 mb-2"><HelpCircle className="text-blue-500 w-5 h-5 flex-shrink-0" /><h3 className="font-bold text-slate-800 text-lg">{faq.question}</h3></div>
       ...
       {/* MODAL FORM (Fixed Colors) */}
       {isFormOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4 overflow-y-auto">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg my-auto">
            <div className="p-6 border-b border-gray-100 flex justify-between items-center bg-gray-50 rounded-t-2xl">
              <h3 className="text-xl font-bold text-slate-800">{editingId ? "Edit" : "Add New"} {activeTab === "posts" ? "Blog Post" : "FAQ"}</h3>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-5">
              ...
              <div className="pt-4 flex gap-4 border-t border-gray-100 mt-4">
                ...
                <button type="submit" className="flex-1 py-3 bg-blue-600 text-white font-bold rounded-xl hover:bg-blue-700 flex items-center justify-center gap-2 transition-colors">
                    <Save size={20} /> Save
                </button>
              </div>
            </form>
