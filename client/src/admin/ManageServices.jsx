...
import {
  Plus, Edit, Trash2, X, Save, Upload, Loader2, Monitor, Smartphone, ShoppingCart, 
  Globe, Server, ShieldCheck, Code2, Database, Cloud, PenTool, Megaphone, Link as LinkIcon
} from "lucide-react";
...
const ManageServices = () => {
  ...
  const [formData, setFormData] = useState({
    title: "", description: "", icon: "Monitor", category: "development", image: "", order: "", link: "",
  });
  ...
  const openAddForm = () => {
    setEditingId(null);
    setFormData({ title: "", description: "", icon: "Monitor", category: "development", image: "", order: "", link: "" });
    setImageFile(null);
    setIsFormOpen(true);
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingId) await axios.put(`${API_URL}/services/${editingId}`, payload);
      else await axios.post(`${API_URL}/services`, payload);
      refreshData();
      setIsFormOpen(false);
    } catch (error) {
    ...
    <div className="relative">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {localItems.map((service) => (
          <div key={service._id} className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex flex-col h-full relative overflow-hidden group hover:shadow-md transition-shadow">
            <div className="relative h-40 bg-gray-100">
               <div className="absolute top-2 left-2 flex items-center gap-1 bg-white/90 px-2 py-1 rounded-lg border border-gray-200 shadow-sm z-10">
                    ...
                    <input type="number" value={service.order || 0} onChange={(e) => handleOrderChange(e, service._id)} className="w-8 text-center bg-white border border-gray-300 rounded text-xs font-bold text-slate-800 focus:outline-none" />
               </div>
               <div className="absolute top-2 right-2 bg-blue-600 px-2 py-1 rounded text-xs font-bold uppercase tracking-wide text-white z-10">{service.category}</div>
               <img src={service.image} alt={service.title} className="w-full h-full object-cover" />
            </div>
            <div className="p-6 flex flex-col flex-grow">
                <div className="flex items-center gap-3 mb-3">
                  <div className={`p-2 rounded-lg bg-blue-50 text-blue-600`}>{renderIcon(service.icon)}</div>
               ...
               <div className="flex gap-3 mt-auto pt-4 border-t border-gray-50">
                <button onClick={() => openEditForm(service)} className="flex-1 py-2 rounded-lg border border-gray-200 text-slate-600 hover:bg-slate-50 flex justify-center items-center gap-2 text-sm font-medium"><Edit size={16} /> Edit</button>
                <button onClick={() => handleDeleteClick(service._id)} className="flex-1 py-2 rounded-lg bg-red-50 text-red-600 hover:bg-red-100 flex justify-center items-center gap-2 text-sm font-medium"><Trash2 size={16} /> Delete</button>
              </div>
            </div>
          </div>
        ))}
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input type="text" name="link" value={formData.link} onChange={handleInputChange} className="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-lg bg-white text-slate-800 focus:ring-2 focus:ring-blue-500 outline-none" placeholder="/contact or https://example.com" />
                </div>
              </div>
              ...
              <div className="pt-4 flex gap-3 border-t border-gray-100 mt-4">
                ...
                <button type="submit" disabled={isUploading} className="flex-1 py-3 bg-blue-600 text-white font-bold rounded-lg hover:bg-blue-700 flex items-center justify-center gap-2 transition-colors">
                  {isUploading ? <><Loader2 className="animate-spin" /> Uploading...</> : <><Save size={18} /> Save Service</>}
                </button>
              </div>
            </form>
          </div>
      ...
      <ConfirmationModal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} onConfirm={confirmDelete} title="Delete Service?" message="Are you sure?" />
    </div>
  );
};
